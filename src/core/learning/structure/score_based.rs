...
impl ScoreBasedLearner {
                    ...
                    for v_name in &nodes {
                        ...
                        if current_model.graph.edges().contains(&(u_id, v_id)) && 
                           !required_edges.contains(&(u_name.clone(), v_name.clone())) {
                            ...
                            let new_v_score = self.local_score(&current_model, v_name, data)?;
                            current_model.graph.add_edge(&u_id, &v_id, u_name, v_name)?; // Backtrack
                        ...
                        // Case 3: Reverse edge U -> V to V -> U
                        if current_model.graph.edges().contains(&(u_id, v_id)) && 
                           !required_edges.contains(&(u_name.clone(), v_name.clone())) &&
    ...
    pub fn ges(
        ...
        loop {
            ...
            for (u_id, v_id) in edges {
                let u_name = current_model.variables().get(&u_id).unwrap().name().to_string();
                let v_name = current_model.variables().get(&v_id).unwrap().name().to_string();
    ...
    pub fn local_score(
        ...
        match self.options.score_type {
            ScoreType::BIC => {
                let mut log_l = 0.0;
                let child_var = model.variables().get(&child_id).unwrap();
                let r = child_var.domain().size().unwrap();
                let q = counts.len() / r;
    ...
    fn get_counts(
        ...
        sorted_parents.sort_by_key(|p| model.variables().get(p).unwrap().name().to_string());
        
        let mut ordered_vars = sorted_parents.iter().map(|id| model.variables().get(id).unwrap()).collect::<Vec<_>>();
        ...
        let parent_sizes: Vec<usize> = sorted_parents.iter().map(|id| model.variables().get(id).unwrap().domain().size().unwrap()).collect();
        let child_domain_size = child_var.domain().size().unwrap();

        for row in data {
            let mut missing = false;
            let mut p_indices = Vec::with_capacity(sorted_parents.len());
            for &p_id in &sorted_parents {
                let p_var = model.variables().get(&p_id).unwrap();
    ...
    fn would_create_cycle(&self, model: &BayesianNetwork, from: &str, to: &str) -> bool {
        let to_id = model.id_of(to).unwrap();
        let from_id = model.id_of(from).unwrap();
