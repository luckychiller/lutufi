...
impl PyVariableEliminationEngine {
    ...
    pub fn query(
        ...
        let var_refs: Vec<&str> = variables.iter().map(|s| s.as_str()).collect();
        let result_factor = engine.query(&var_refs, &rust_evidence, h).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
impl PyLBPEngine {
    ...
    pub fn query(
        ...
        let fg = FactorGraph::from_bayesian_network(&model.inner).map_err(|e| PyValueError::new_err(e.to_string()))?;
        ...
        let res = engine.run(&rust_evidence).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
impl PyVariationalEngine {
    ...
    pub fn query(
        ...
        let res = engine.run_with_evidence(&rust_evidence).map_err(|e| PyValueError::new_err(e.to_string()))?;
