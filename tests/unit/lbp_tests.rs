use lutufi_core::core::{
    ...
    inference::{lbp::{LBPEngine, LBPOptions}, junction_tree::JunctionTreeEngine},
};
...
fn test_lbp_asia() {
    ...
    let mut engine = LBPEngine::new(fg, LBPOptions::default());
    let evidence = Assignment::new();

    let result = engine.run(&evidence).unwrap();
    ...
    let lbp_asia = result.beliefs.get(&asia_id).unwrap();
    assert_abs_diff_eq!(jt_asia.value_at(1), lbp_asia.value_at(1), epsilon = 0.01);
}
