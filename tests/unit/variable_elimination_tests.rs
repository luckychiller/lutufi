...
mod tests {
    ...
    use lutufi_core::core::{
        ...
        inference::variable_elimination::{VariableEliminationEngine, EliminationHeuristic},
    };
    ...
    #[test]
    fn test_ve_asia_no_evidence() {
        let bn = build_asia_network();
        let engine = VariableEliminationEngine::new(&bn);
        let evidence = Assignment::new();

        // P(Asia) = [0.99, 0.01]
        let res = engine.query(&["Asia"], &evidence, EliminationHeuristic::MinFill).unwrap();
        assert_abs_diff_eq!(res.value_at(0), 0.99, epsilon = 1e-10);
        assert_abs_diff_eq!(res.value_at(1), 0.01, epsilon = 1e-10);

        // P(Tuberculosis) = [0.9896, 0.0104]
        let res = engine.query(&["Tuberculosis"], &evidence, EliminationHeuristic::MinFill).unwrap();
        assert_abs_diff_eq!(res.value_at(1), 0.0104, epsilon = 1e-10);

        // P(LungCancer) = [0.945, 0.055]
        let res = engine.query(&["LungCancer"], &evidence, EliminationHeuristic::MinFill).unwrap();
        assert_abs_diff_eq!(res.value_at(1), 0.055, epsilon = 1e-10);
    }

    #[test]
    fn test_ve_asia_with_evidence() {
        let bn = build_asia_network();
        let engine = VariableEliminationEngine::new(&bn);
        ...
        let res = engine.query(&["Tuberculosis"], &evidence, EliminationHeuristic::MinFill).unwrap();
        assert_abs_diff_eq!(res.value_at(0), 0.95, epsilon = 1e-10);
        assert_abs_diff_eq!(res.value_at(1), 0.05, epsilon = 1e-10);
    }
}
